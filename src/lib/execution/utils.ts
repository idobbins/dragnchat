 ...
 */
export function parseNodesToDAG(nodes: CustomNode[], edges: CustomEdge[]): ExecutionGraph {
 ...
 */
export function validateExecutionGraph(graph: ExecutionGraph): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
export function getNodeInputData(nodeId: string, graph: ExecutionGraph): string {
 ...
 */
export function isNodeReadyToExecute(nodeId: string, graph: ExecutionGraph): boolean {
  const node = graph.nodes.get(nodeId);
